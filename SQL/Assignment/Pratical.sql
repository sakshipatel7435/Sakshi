...
select customer_id,  max(purch_amt) as total_amount
from orders 
...
-- Exclude employees of the Jobtitle ‘MK_MAN’. Return employee ID, first name, last name, job ID. 
select employee_id, first_name, last_name, job_id, salary
from employees
where salary < (select min(salary) from employees where job_id = 'mk_man') and job_id <> 'mk_man';

-- 9. write a SQL query to find all those employees who work in department ID 80 or 40. Return first name, last name, department number 
-- and department name. 
...
-- 13.write a SQL query to find all those employees who work in the Finance department. Return department ID, 
-- name (first), job ID and department name. 
select e.department_id, first_name, job_id, d.department_name
from employees e
join departments d
on e.department_id = d.department_id
where d.department_name = 'finance';

-- 14. From the following table, write a SQL query to find the employees who earn less than the employee of ID 182. Return first name,
-- last name and salary.
select first_name, last_name, salary
from employees
where salary < (select min(salary) from employees where employee_id = 182);
...
BEGIN
    SELECT 
        department_id,
        COUNT(employee_id) AS total_number_of_employee
    FROM employees
...
CREATE PROCEDURE PromoteEmployee(
    ...
    IN p_new_job_role VARCHAR(50)
)
BEGIN
    UPDATE employees
    SET salary = p_new_salary,
        job_role = p_new_job_role
